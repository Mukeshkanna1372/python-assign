...
def add_task(value):
    ...
    if task != "":
        if values[0]:
            to_do_list.insert(0, task+" *Morning ")
        elif values[1]:
            to_do_list.insert(int(len(to_do_list)/2)+1, task+" *Afternoon ")
        elif values[2]:
            to_do_list.insert(len(to_do_list), task+" *Night ")
...
layout = [
    [
        Sg.Text("Enter Medicine Name", font=(font_name, font_size)),
    ...
    [
        ...
        Sg.Radio("MORNING", 1),
        Sg.Radio("AFTERNOON", 1, default=True),
        Sg.Radio("NIGHT", 1)
    ],
